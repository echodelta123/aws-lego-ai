...
-----------------------
Thin wrapper around the SageMaker real-time inference endpoint for Graph Neural
Network (LightGCN) link prediction based LEGO product recommendations.

Responsibilities:
  - Enforce input schema before sending to the endpoint
  - Parse and validate the endpoint's response (user-item bipartite link prediction scores)
...
ENDPOINT_NAME = os.environ.get("SAGEMAKER_ENDPOINT_NAME", "lego-graph-recommender-endpoint")
